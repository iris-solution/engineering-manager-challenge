...
- Nothing in the plan depends on which cloud. Two things do follow: config-in-Git is the biggest lever on the region-4 date, and every background job needs its own connection budget
...
- Bets can be unsettled, void or cancelled — the pack is silent, so I asked in the review rather than guessing
...
**Lengths.** C1 is 299 words and C2 is 298, both inside the 300-word limit. Part A is ~1,840 words and Part B a 640-word review plus a 340-word answer — and roughly half of that sits inside tables and comment blocks, which print far denser than prose. Appendices are outside the limits.
